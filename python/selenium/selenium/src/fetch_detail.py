...
def get_house_detail():
    ...
    for i in range(0, len(url_list)):
        ...
        try:
            ...
            continue
        workbook_sheet.write(int(i + 1), int(i), total_price + '万')
        workbook_sheet.write(int(i + 1), int(i + 1), unit_price + '元/m' + '\u00b2')
        workbook_sheet.write(int(i + 1), int(i + 2), community_name)
        workbook_sheet.write(int(i + 1), int(i + 3), locate_area)
        workbook_sheet.write(int(i + 1), int(i + 4), lon)
        workbook_sheet.write(int(i + 1), int(i + 5), lat)
        workbook_sheet.write(int(i + 1), int(i + 6), house_type)
        workbook_sheet.write(int(i + 1), int(i + 7), house_floor)
        workbook_sheet.write(int(i + 1), int(i + 8), house_construction_area)
        workbook_sheet.write(int(i + 1), int(i + 9), house_structure)
        workbook_sheet.write(int(i + 1), int(i + 10), house_inside_area)
        workbook_sheet.write(int(i + 1), int(i + 11), house_building_type)
        workbook_sheet.write(int(i + 1), int(i + 12), house_facing)
        workbook_sheet.write(int(i + 1), int(i + 13), building_structure)
        workbook_sheet.write(int(i + 1), int(i + 14), renovation_condition)
        workbook_sheet.write(int(i + 1), int(i + 15), elevator_ratio)
        workbook_sheet.write(int(i + 1), int(i + 16), heating_method)
        workbook_sheet.write(int(i + 1), int(i + 17), equipped_with_elevator)
        workbook_sheet.write(int(i + 1), int(i + 18), listing_time)
        workbook_sheet.write(int(i + 1), int(i + 19), transaction_ownership)
        workbook_sheet.write(int(i + 1), int(i + 20), last_transaction)
        workbook_sheet.write(int(i + 1), int(i + 21), usage_of_houses)
        workbook_sheet.write(int(i + 1), int(i + 22), years_of_housing)
        workbook_sheet.write(int(i + 1), int(i + 23), property_rights)
        workbook_sheet.write(int(i + 1), int(i + 24), mortgage_information)
        workbook_sheet.write(int(i + 1), int(i + 25), room_spare_parts)
