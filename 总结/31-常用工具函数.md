...
#### 17. **驼峰命名转横线命名：**拆分字符串，使用 - 相连，并且转换为小写
...
#### 27. **生成一个重复的字符串，有n个str组成，可修改为填充为数组等**
...
#### 42. **使用 value 值来填充 array，从start位置开始, 到end位置结束（但不包含end位置），返回原数组**
...
#### 43. **用来判断一个数组是否包含一个指定的值，如果是返回 true，否则false，可指定开始查询的位置**
...
#### 45. **返回数组中通过测试（函数fn内判断）的第一个元素的下标**
