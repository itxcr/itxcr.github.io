...
可以使用 new 操作符来生成一个 Perosn 类的实例：

```ts
const person = new Person('xcr', 'xcr01')
```

运行时不会遵循基于类的继承模型。当使用 new 操作符时，运行时会创建一个新对象，并让其继承自 Person 类的原型。

可以认为运行时（JS）的 new 操作符的行为和程序设计阶段（TS）的 extends 关键字的行为没有区别。

### 闭包

闭包是最强大的运行时特性之一，但同时它也是被误解得最多的。Mozilla 开发者文档对比包有如下定义：

- 闭包是指向独立变量的函数。换句话说，在闭包中定义的函数会“记住”它创建时的环境

可以将独立变量理解为在我们创建的字面作用域上持续存在的变量。来看一个例子：

```js
function makeArmy() {
    const shooters = []
    for (var i = 0; i < 10; i++) {
        const shooter = function () { // 一个shooter即一个函数
            alert(i) // 将弹出一个数字
        }
...
声明了一个名为 makeArmy 的函数。在函数内部，创建一个名为 shotters 的数组，其元素都是函数。shotters 数组中的每一个函数都将弹出一个数字，数字值为存储在函数内部的 i 值。现在调用这个 makeArmy 函数：

```js
const army = makeArmy()
```

现在，army 变量包含了一个函数 shotter 的数组，但是执行其中的函数会发现问题：
...
之所以上述代码没有按照预期工作，是因为犯了一个普遍的关于闭包的错误。当在 makeArmy 函数中声明一个 shotter 函数时，在无意中，创建了一个闭包。

造成上述情况的原因是赋值给 shotter 的函数都是闭包。它们包含自身的函数定义并保存了 makeArmy 的作用域环境。创建出了 10 个闭包，但它们共享了同一个环境。当 shotter 函数被执行时，函数中的循环已经执行完毕，i 变量的值也变成 10.
...
上述代码运行符合预期。我们不再简单共享同一个环境，而是使用一个自执行函数创建一个新的作用域环境，在其中，i 指向的是对应的期望值。

### 闭包和静态变量

当一个变量在一个闭包环境中被声明后，它可以在一个类的不同实例间共享，换句话说，这个变量表现得像一个静态变量。

先创建一个名为 Coounter 的 TS 类：
...
上述类中包含了一个名为 _COUNTER 的静态成员。TS 编译器会生成如下代码：
...
静态变量被 TS 编译器声明为了 类属性（不是实例属性）。编译器之所以使用类属性是因为它不会被所有实例共享。

或者，也可以使用 JS 代码（所有合法的 JS 代码都是合法的 TS 代码）用闭包来模拟静态属性：
...
上面代码声明了一个名为 Counter 的类。这个类包含一些方法来增大、减小或读取 _COUNTER 变量的值。`_COUNTER` 变量本身并不是对象原型的一部分。

Counter 构造函数自身就是一个闭包的一部分。所以，所有 Counter 类的实例都共享同一个闭包上下文，这意味着这个上下文中的 counter 变量和 changeBy 函数将会表现的如单例一样。
...
console.log(counter2.value()) // 2 期望 0
counter1.decrement()
console.log(counter1.value()) // 1
console.log(counter2.value()) // 1 期望 0
```

### 闭包和私有成员

已经看到闭包函数可以访问到在创建的字面作用域上持续存在的变量。这些变量并不是函数原型或函数体中的一部分，但它们是闭包上下文中的一部分。

由于不可以直接访问闭包上下文，那么该上下文中的变量和成员就可以用来模拟私有成员。使用闭包来模拟私有成员（不是使用 TS 的 private 描述符）的主要好处是闭包会在运行时组织对它们的访问。

TS 在运行时并不会模拟私有变量。如果试图访问一个私有成员，那么 TS 编译器会在编译时抛出一个错误。

TS 由于性能原因，并没有在运行时使用闭包模拟私有成员。不管我们添加和删除 private 描述符，生成的 JS 代码都不会有变化。这意味着在运行时，私有成员变成了公开成员。

但是，使用闭包在运行时模拟私有变量是可行的。就像使用闭包来模拟静态变量一样，也可以使用纯 JS 来完成成员的可见控制性。看一个例子：
...
counter1.increment()
console.log(counter1.value()) // 2
console.log(counter2.value()) // 0 期望 0
counter1.decrement()
console.log(counter1.value()) // 1
console.log(counter2.value()) // 0 期望 0
```

由于闭包上下文不能直接被访问，因此可以说变量 _COUNTER和函数 changeBy 是私有成员：

```js
console.log(counter1._COUNTER) // undefined
console.log(counter1.value()) // 1
console.log(counter1.changeBy(2)) //  counter1.changeBy is not a function      
```
