  ...
  <div class="message">
    <div class="message-header">
      <div class="message-header-box">
        ...
        <general-active v-if="groupGather[activeRoom.groupId]" type="group"></general-active>
        <general-active v-else type="friend"></general-active>
      </div>
    </div>
      ...
      <div class="message-content">
        ...
        <template v-for="item in activeRoom.messages">
          <div class="message-content-message" :key="item.userId + item.time" :class="{ 'text-right': item.userId === user.userId }">
            <general-avatar :data="item"></general-avatar>
            <div>
...
import GeneralActive from '@/components/GeneralActive.vue'
...
export default class GeneralMessage extends Vue {
  ...
  changeActiveRoom() {
    ...
    if (this.activeRoom.messages && this.activeRoom.messages.length > 30) {
      this.activeRoom.messages = this.activeRoom.messages.splice(this.activeRoom.messages.length - 30, 30) as GroupMessage[] & FriendMessage[]
    }
    this.scrollToBottom()
  }
  ...
  addMessage() {
    if (this.activeRoom.messages) {
      ...
      if (messages[messages.length - 1].userId === this.user.userId || (this.messageDom && this.messageDom.scrollTop + this.messageDom.offsetHeight + 100 > this.messageContentDom.scrollHeight)) {
        this.scrollToBottom()
      }
    }
  }
